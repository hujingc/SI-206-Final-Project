...
cur = conn.cursor()
cur.execute(
    'CREATE TABLE IF NOT EXISTS NYTHome (title TEXT, author TEXT, published TIMESTAMP, section TEXT)')
...
for news in res['results'][start:start+20]:
    ...
    cur.execute('INSERT INTO NYTHome (title, author, published, section) VALUES (?, ?, ?, ?)',
